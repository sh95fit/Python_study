...
class AuthTest(TestCase):
    ...
    def test_register(self):
        ...
        body = {"email": "test@test.com", "name": "Test User",
                "password": "1234123", "policy": True}
        res = c.post("/ninja-api/users/register", json.dumps(body),
                     content_type="application/json")
        # password length < 7
        self.assertEqual(res.status_code, 422)

        body = {"email": "testest.com", "name": "Test User",
                "password": "12341235", "policy": True}
        res = c.post("/ninja-api/users/register", json.dumps(body),
                     content_type="application/json")
        # email validation
        self.assertEqual(res.status_code, 422)
        ...
        body = {"email": "test@test.com", "name": "Test User",
                "password": "12341235", "policy": False}
        res = c.post("/ninja-api/users/register", json.dumps(body),
                     content_type="application/json")
        # policy validation
        self.assertEqual(res.status_code, 422)
        ...
        body = {"email": "test1@test.com", "name": "Test User",
                "password": "12341235", "policy": True}
        res = c.post("/ninja-api/users/register", json.dumps(body),
                     content_type="application/json")
        # Email Duplicate
